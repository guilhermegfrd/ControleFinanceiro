...
namespace ControleFinanceiro.DAL.Repositorios
{
    public class UsuarioRepositorio : RepositorioGenerico<Usuario>, IUsuarioRepositorio
    {
        ...
        private readonly SignInManager<Usuario> _gerenciadorLogins;
        public UsuarioRepositorio(
            Contexto contexto,
            UserManager<Usuario> gerenciadorUsuarios,
            SignInManager<Usuario> gerenciadorLogins

            ) : base(contexto)
        {
            _contexto = contexto;
            _gerenciadorUsuarios = gerenciadorUsuarios;
            _gerenciadorLogins = gerenciadorLogins;
        }
        ...
        public async Task LogarUsuario(Usuario usuario, bool lembrar)
        {
            try
            {
                await _gerenciadorLogins.SignInAsync(usuario, false);
            }
            catch (Exception ex)
            {
